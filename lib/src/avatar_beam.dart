...
class AvatarBeamData {
  ...
  double wrapperRotate;
  double wrapperScale;
  double wrapperRadius;
  bool isMouthOpen;
  bool isCircle;
  double eyeSpread;
  double mouthSpread;
...
class AvatarBeamPainter extends AvatarCustomPainter {
  ...
  AvatarBeamPainter(this.name, this.colors)
      : propertie = generate(name, colors);

  static AvatarBeamData generate(String name, List<Color> colors) {
    const double boxSize = 36;
    final numFromName = getNumber(name);
    final range = colors.length;

    final wrapperColor = getRandomColor<Color>(numFromName, colors, range);
    ...
    final isCircle = getBoolean(numFromName, 1);

    return AvatarBeamData(
      wrapperColor: wrapperColor,
      faceColor: getContrast(wrapperColor),
      backgroundColor: getRandomColor(numFromName + 13, colors, range),
      wrapperTranslateX: wrapperTranslateX.toDouble(),
      wrapperTranslateY: wrapperTranslateY.toDouble(),
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final wrapperTransform = getTransform(translateX: p.wrapperTranslateX, translateY: p.wrapperTranslateY, rotate: p.wrapperRotate, scale: p.wrapperScale);
    canvas.transform(wrapperTransform.storage);
    final wrapperRect = getRectFromLTWH(0, 0, 36, 36);
    canvas.drawRRect(RRect.fromRectXY(wrapperRect, cX(p.wrapperRadius), cY(p.wrapperRadius)), fillPaint(p.wrapperColor));
    wrapperTransform.invert();
    canvas.transform(wrapperTransform.storage);
    canvas.transform(getTransform(translateX: p.faceTranslateX, translateY: p.faceTranslateY, rotate: p.faceRotate).storage);
    if(p.isMouthOpen) {
    ...
    canvas.drawRRect(RRect.fromRectXY(lEyeRect, cX(1), cX(1)), fillPaint(p.faceColor));
    final rEyeRect = getRectFromLTWH(20 + p.eyeSpread, 14, 1.5, 2);
    canvas.drawRRect(RRect.fromRectXY(rEyeRect, cX(1), cX(1)), fillPaint(p.faceColor));
